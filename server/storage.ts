...
export class DatabaseStorage implements IStorage {
  ...
  async createLicenseKey(applicationId: number, license: InsertLicenseKey): Promise<LicenseKey> {
    ...
    // Calculate expiration: use provided expiresAt, calculate from validityDays, or set to null for unlimited
    let expiresAt: number | null = null;
    let validityDays: number = 0; // Default to 0 for unlimited
    ...
    if (license.expiresAt) {
      ...
      // If neither expiresAt nor validityDays provided, create unlimited license
      expiresAt = null;
      validityDays = 0;
    }
  ...
  async validateLicenseKey(licenseKey: string, applicationId: number): Promise<LicenseKey | null> {
    ...
    // Check if license has expired (null means unlimited validity)
  ...
  async createWebhook(userId: string, webhook: any): Promise<Webhook> {
    ...
    // Convert events back to array for response
    return {
      ...newWebhook,
  ...
  async getUserWebhooks(userId: string): Promise<Webhook[]> {
    ...
    // Convert events back to arrays for response
