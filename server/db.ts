...
const sqlite = new Database('./database/primeauth.sqlite');
...
const initializeDatabase = () => {
  ...
  try {
    ...
    // Migration to allow null expires_at in license_keys table for unlimited validity
    try {
      // Check if we need to migrate the license_keys table
      const checkColumn = sqlite.prepare(`PRAGMA table_info(license_keys)`).all();
      const expiresAtColumn = checkColumn.find(col => col.name === 'expires_at');
      
      if (expiresAtColumn && expiresAtColumn.notnull === 1) {
        console.log('Migrating license_keys table to allow unlimited validity...');
        
        // Create new license_keys table with nullable expires_at
        const migrateLicenseKeys = `
          -- Create backup table
          CREATE TABLE license_keys_backup AS SELECT * FROM license_keys;
          
          -- Drop the old table
          DROP TABLE license_keys;
          
          -- Create new table with nullable expires_at
          ...
          INSERT INTO license_keys (id, application_id, license_key, max_users, current_users, validity_days, expires_at, is_active, description, created_at, updated_at)
          SELECT id, application_id, license_key, max_users, current_users, validity_days, expires_at, is_active, description, created_at, updated_at
          FROM license_keys_backup;
          
          -- Drop backup table
          DROP TABLE license_keys_backup;
        `;
        
        sqlite.exec(migrateLicenseKeys);
        console.log('License keys table migration completed - unlimited validity now supported!');
      }
    } catch (error) {
