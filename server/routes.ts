...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/auth/simple-login', async (req: any, res) => {
    try {
      ...
      // Find user in JSON file by username or email
      ...
      console.log('Session created and saved successfully. Session data:', req.session);
      console.log('Session user data:', (req.session as any).user);
  ...
  app.post('/api/applications/:id/rotate-api-key', isAuthenticated, async (req: any, res) => {
    try {
      const applicationId = parseInt(req.params.id);
      const application = await storage.getApplication(applicationId);
      ...
      const newApiKey = 'pa_' + Array.from(
        { length: 32 }, 
      ...
      const updatedApplication = await storage.updateApplication(applicationId, { apiKey: newApiKey });
      
      if (!updatedApplication) {
        return res.status(404).json({ message: "Application not found" });
      }

      res.json({ success: true, message: "API key rotated successfully", apiKey: newApiKey });
    } catch (error) {
  ...
  app.post('/api/applications/:id/licenses/generate', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Validate that expiresAt is in the future if provided (null/undefined means unlimited)
      if (expiresAt) {
  ...
  // Registration disabled - users can only login with credentials from userauth.json
  app.post('/api/auth/register', async (req: any, res) => {
    res.status(403).json({ 
      success: false, 
      message: "Registration is disabled. Please contact administrator for access." 
    });
  });
  ...
  app.post('/api/auth/update-username', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Find the user by UID and verify current password
      const userIndex = userAuthData.findIndex((user: any) => 
        user.uid === userId && user.password === currentPassword
      );
      ...
      // Update username in userauth.json
      userAuthData[userIndex].username = username;
      writeFileSync(userAuthPath, JSON.stringify(userAuthData, null, 2));

      // Update user in database
      await storage.updateUser(userId, { firstName: username });
  ...
  app.post('/api/auth/update-email', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Update email in userauth.json
      userAuthData[userIndex].email = email;
      writeFileSync(userAuthPath, JSON.stringify(userAuthData, null, 2));

      // Update user in database
      await storage.updateUser(userId, { email: email });
  ...
  app.post('/api/auth/update-password', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Update password in userauth.json
      userAuthData[userIndex].password = newPassword;
      writeFileSync(userAuthPath, JSON.stringify(userAuthData, null, 2));
