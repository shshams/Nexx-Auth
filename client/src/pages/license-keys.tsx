...
import { Plus, Trash2, Copy, Key, Users, Calendar, Zap, CheckSquare, ArrowLeft } from "lucide-react";
...
export default function LicenseKeys() {
  ...
  const handleCreateLicense = () => {
    ...
    // Only validate expiration date if it's provided (empty means unlimited)
  ...
  const isExpired = (expiresAt: string | null | undefined) => {
    // Null or undefined means unlimited validity (never expires)
    if (!expiresAt) return false;
    return new Date(expiresAt) < new Date();
  };

  const getRemainingDays = (expiresAt: string | null | undefined) => {
    // Null or undefined means unlimited validity
            ...
            <div className="flex gap-2">
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="gen-expires-at">Expires At (Optional)</Label>
                    <Input
                      id="gen-expires-at"
                      type="datetime-local"
                    ...
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      Leave empty for unlimited validity (never expires)
                    </p>
                  </div>
                  <div>
                        ...
                        <TableCell>
                          <div className="flex flex-col">
                            <span className="text-sm">
                              {license.expiresAt ? new Date(license.expiresAt).toLocaleDateString() : "Never expires"}
                            </span>
