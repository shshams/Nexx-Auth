...
import { Copy, Settings, ArrowLeft, Users, Activity, Eye, EyeOff, MoreHorizontal, Trash2, Pause, Play, Key, Shield, Plus, UserPlus, CheckSquare, RotateCcw } from "lucide-react";
...
export default function AppManagement() {
  ...
  const rotateApiKeyMutation = useMutation({
    mutationFn: () => 
      apiRequest(`/api/applications/${appId}/rotate-api-key`, {
        method: 'POST',
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/applications'] });
      queryClient.invalidateQueries({ queryKey: [`/api/applications/${appId}`] });
      queryClient.invalidateQueries({ queryKey: [`/api/applications/${appId}/stats`] });
      toast({ title: "API key rotated successfully", description: "New API key has been generated" });
    },
    onError: (error: any) => {
      toast({ 
